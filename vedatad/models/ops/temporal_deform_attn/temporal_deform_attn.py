...
class DeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_temporal_lens, input_level_start_index, input_padding_mask=None):
        ...
        #下面的备注都是deformable detr中，四层特征的结果，除了部分2我改成1了
        ...
        value = value.view(N, Len_in, self.n_heads,
                           self.d_model // self.n_heads) #(N,Len_in,8,64)
        # the predicted offset in temporal axis. They are *absolute* values, not normalized
        sampling_offsets = self.sampling_offsets(query).view(
            N, Len_q, self.n_heads, self.n_levels, self.n_points, 1) #(N,Len_q,8,4,4,1) 预测采样点的坐标偏移
        attention_weights = self.attention_weights(query).view(
            N, Len_q, self.n_heads, self.n_levels * self.n_points) #(N,Len_q,8,4*4) 预测采样点对应的注意力权重
        attention_weights = F.softmax(
            attention_weights, -1).view(N, Len_q, self.n_heads, self.n_levels, self.n_points) #(N,Len_q,8,4，4) 每个query在每个注意力头内，每个特征层都采样4个特征点，共4x4=16个，对这16个权重进行归一化
        ...
        if reference_points.shape[-1] == 1:
            ...
            #对坐标偏移量使用对应层特征图的宽高进行归一化，然后在参考点坐标上得到采样点坐标
